...
async function bootContext(
  ...
  for (const id of REQUIRED) {
    const entry = [...loader.entries()].find(
      (item) => item.options.name === `cordis:${id}`,
    );
...
function BootApp() {
  ...
  const [endpoint, setEndpoint] = useState<HarnessEndpointInfo | undefined>(
    undefined,
  );

  useEffect(() => {
    let cancelled = false;
    void bootContext(host.platform, host.account).then(
      (next) => {
        if (cancelled) {
  ...
  if (error !== undefined) {
    return (
      <div className="grid h-full place-items-center p-8 text-[13px] text-danger">
        {error}
      </div>
    );
  }
