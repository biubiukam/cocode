...
export function bindSnapshotSelector<T>(w: HostObservable<T>): SnapshotSelectorHook<T> {
  ...
  return function useSelector<S>(sel: (s: T) => S, eq?: (a: S, b: S) => boolean): S {
    ...
    const previous = useRef<{ snapshot: T; selected: S } | undefined>(undefined)
    const selected = sel(snapshot)
    if (previous.current !== undefined && Object.is(previous.current.snapshot, snapshot)) {
      return previous.current.selected
    }
    if (previous.current !== undefined && eq?.(previous.current.selected, selected)) {
      previous.current = { snapshot, selected: previous.current.selected }
      return previous.current.selected
    }
    previous.current = { snapshot, selected }
    return selected
  }
}
