...
const DEV_DEPENDENCY_ALIASES = Object.fromEntries(
  ['@tanstack/react-virtual', 'diff']
    .map(specifier => [specifier, resolveDevDependency(specifier)] as const)
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    // These vendored packages are workspace members under `vendor/`, not
    // installed in the root node_modules tree. Resolve them to their source
    // entries so `alwaysBundle` has a concrete module graph to inline.
    alias: {
      '@deepseek-ai/cosmokit': resolvePath(REPOSITORY_ROOT, 'vendor/cosmokit/src/index.ts'),
      '@deepseek-ai/schemastery': resolvePath(REPOSITORY_ROOT, 'vendor/schemastery/src/index.ts'),
