 ...
 * solid configured or missing dots. Official DeepSeek is a whole-section
 * route: it joins the added list only after a user overlay or stored key
 * exists, and it can be deleted like any other user-added provider. The add
 * flow is a card carrying the dormant-provider select. Each card kind owns
 * its own open state, so closing one never discards a draft in another.
 * Every mutation writes through the wire, while a provider removal first
 * requires confirmation; the page re-renders from pushed invalidations or
...
/** Render an editor for an expanded provider row. */
...
/**
 * The credential this page wrote for the row, when deleting the row should
 * take that key with it. Nested routes store under the derived reference;
 * a whole-section adapter names its conventional env in the schema instead
 * (DeepSeek: `DEEPSEEK_API_KEY`, not `DEEPSEEK_OFFICIAL_API_KEY`).
 */
function pageManagedCredentialRef(row: ProviderRow): string | undefined {
  if (row.apiKeyEnv === undefined) return undefined
  if (row.credential?.configured !== true || row.credential.writable !== true) return undefined
  if (row.apiKeyEnv === deriveKeyRef(row.entry.provider) || row.entry.settingsPath.length === 0) {
    return row.apiKeyEnv
  }
  return undefined
}

function targetOf(row: ProviderRow): EditorTarget {
  const credentialRef = pageManagedCredentialRef(row)
  return {
