...
/** Marker read by the Electron bootstrap bridge before the client graph mounts. */
function bootThemeMeta(preference: ThemePreference): string {
  return `<meta name="dsh-theme-preference" content="${preference}">`
}
...
export function injectBootTheme(
  ...
  const meta = bootThemeMeta(preference)
