...
describe('session cwd resolution over the API route', () => {
  ...
  it('fs.read resolves repo-relative paths (untracked diff fallback)', async () => {
    const repoRoot = await git.repoRoot(process.cwd())
    const repoRelativePath = relative(repoRoot, join(process.cwd(), 'src/git.ts'))
    const route = mount({
      sessions: {
        get: () => ({ header: { cwd: join(process.cwd(), 'src') } }),
      },
    })
    const result = await invoke(route, 'fs.read', { sessionId: 's-sub', path: repoRelativePath })
    expect(result.ok).toBe(true)
