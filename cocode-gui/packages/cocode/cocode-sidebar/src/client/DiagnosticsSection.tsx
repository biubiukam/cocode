...
type DiagnosticsApi = {
  ...
  readonly exportBundle: () => Promise<{ readonly cancelled: boolean; readonly fileName?: string; readonly bytes?: number }>
  readonly clearLogs: () => Promise<void>
  readonly enableTemporaryDebug: (request: { readonly durationMinutes: 30 | 60 }) => Promise<{ readonly enabledUntil: string }>
}

declare global {
  interface Window {
    readonly desktopApi?: { readonly diagnostics?: DiagnosticsApi }
  }
}
...
export function DiagnosticsSection(): JSX.Element {
  ...
  const [status, setStatus] = useState<DiagnosticsStatus | undefined>()
