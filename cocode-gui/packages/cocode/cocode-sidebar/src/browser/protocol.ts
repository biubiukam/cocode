...
export type BrowserClientFrame =
  ...
  | { t: 'nav'; to: 'back' | 'forward' | 'reload' | 'stop' }
  /** The canvas size changed; the host resizes the page viewport to match. */
  | { t: 'viewport'; width: number; height: number; dpr: number }
  | {
    t: 'mouse'
...
export type BrowserServerFrame =
  | { t: 'state'; state: BrowserTabState }
  | { t: 'engine'; status: BrowserEngineStatus }
  | { t: 'dialog'; dialog: BrowserDialog | null }
  | { t: 'download'; name: string; path: string }
  | { t: 'copy'; text: string }
  | { t: 'error'; code: string; message: string }

// ── Agent-facing snapshot vocabulary ────────────────────────────────────────
...
export type BrowserAction =
  ...
  | { kind: 'navigate'; to: 'back' | 'forward' | 'reload' }
  | { kind: 'upload'; ref: string; paths: readonly string[] }
  | { kind: 'dialog'; accept: boolean; text?: string }
  | { kind: 'wait'; condition: 'load' | 'network-idle' | 'text' | 'url'; value?: string }
...
export const IDEMPOTENT_ACTIONS: readonly BrowserAction['kind'][] = ['hover', 'scroll', 'wait']
