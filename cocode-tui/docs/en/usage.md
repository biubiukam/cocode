...
- The status line shows the latest assistant input/output usage and current subagent activity when the wire reports it. When optional events are present, it also shows decode TPS, cache hit rate, context-window percentage, reasoning effort, and the current working activity. Context segments are retained in the runtime snapshot for later visualization; their values are estimates based on text length, not provider billing data.
