...
- Mouse tracking stays off during normal conversation, including when the wide Inspector is visible, so Terminal/iTerm can keep native drag-selection. Popup controls may temporarily enable tracking; models, commands, questions, and message actions remain keyboard accessible. Open the command menu with `Ctrl+P`, or press `Shift+↑` and then `m` for message actions.
...
- `/model` and `/models` without arguments open the model picker; `/model <model-id>` switches the current provider directly. The picker can switch provider and model together. When the restarted runtime supports durable session open, the current session is reopened so its context remains available; otherwise the TUI starts a new session. A failed switch attempts to restore the previous provider/model. Older runtimes without a model catalog still accept a manually entered model id. Choosing a model that advertises reasoning efforts continues to a second step for the effort level.
- `/effort` without arguments opens the current model's reasoning-effort picker; `/effort <level>` sets a level from the Host catalog; `/effort auto` or `/effort default` restores the model default. The TUI does not hard-code low/medium/high. The composer title shows the current level after the model name. `/thinking` still only toggles thinking display and does not change effort.
...
| `/effort`                      | Open the current model's reasoning-effort picker                                                 |
| `/effort <level>`              | Set reasoning effort; `auto` / `default` restores the model default                              |
...
| `/login` / `/logout`           | Sign in or out of Cocode Nut; logout keeps your key and stays in chat                           |
