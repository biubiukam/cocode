...
自己的 API Key 通过首屏登录直接写入共享 DSH 凭据文件 `~/.dsh/.credentials.yaml`，TUI 不新增单独的 API Key 环境变量，也不从旧的 `.cocode/credentials` 自动迁移。`COCODE_HOME` 只负责账号和 runtime；`COCODE_DSH_HOME` 负责共享 settings、credentials、profile 插件和会话。会话目录默认使用 `COCODE_DSH_HOME/sessions`，未设置时使用 `~/.dsh/sessions`，也可以用 `DSH_SESSION_ROOT` 覆盖。
