...
import { applyMousePointerShape, createMouseDecoder, enableMouseTracking, isMousePointerEvent, isMouseInput, layoutRowFromMouseY, mousePointerForTranscript, mousePointerSequence, mouseWheelDelta, shouldEnableMouseTracking } from '../../src/present/mouse.ts'
...
describe('terminal mouse support', () => {
  ...
  it('enables and disables SGR tracking', () => {
    ...
    leave()
    expect(write.mock.calls).toEqual([['\u001b[?1003h\u001b[?1006h'], ['\u001b]22;default\u0007\u001b[?1003l\u001b[?1006l']])
  })

  it('enables transcript wheel and simulated selection when supported', () => {
    expect(
  ...
  it('emits OSC 22 sequences for pointer shapes', () => {
    expect(mousePointerSequence('text')).toBe('\u001b]22;text\u0007')
    expect(mousePointerSequence('default')).toBe('\u001b]22;default\u0007')
  })

  it('uses the text pointer over selectable transcript', () => {
    expect(
  ...
  it('writes OSC 22 only when the pointer shape changes', () => {
    const write = vi.fn()
    expect(applyMousePointerShape({ write }, 'text')).toBe('text')
    expect(applyMousePointerShape({ write }, 'text', 'text')).toBe('text')
    expect(applyMousePointerShape({ write }, 'default', 'text')).toBe('default')
    expect(write.mock.calls).toEqual([['\u001b]22;text\u0007'], ['\u001b]22;default\u0007']])
  })
})
