...
describe("terminal mouse support", () => {
  it("decodes SGR press, release, modifiers, and wheel events", () => {
    ...
    decoder.feed(
      "\u001b[<0;12;8M\u001b[<20;4;3m\u001b[<64;10;11M\u001b[<35;6;7M",
    );
  ...
  it("maps pressed wheel events to transcript movement", () => {
    ...
    expect(
      mouseWheelDelta({ action: "release", button: "wheel-up" }),
    ).toBeUndefined();
  ...
  it("writes OSC 22 only when the pointer shape changes", () => {
    ...
    expect(applyMousePointerShape({ write }, "default", "text")).toBe(
      "default",
    );
    expect(write.mock.calls).toEqual([
      ["\u001b]22;text\u0007"],
