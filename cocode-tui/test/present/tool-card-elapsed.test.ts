...
describe('running tool elapsed clock', () => {
  ...
  it('does not keep ticking elapsed time after a tool is cancelled', async () => {
    vi.useFakeTimers({
      toFake: [
        'Date',
        'setInterval',
    ...
    await vi.advanceTimersByTimeAsync(0)
    expect(visible(stdout.output)).toContain('canceled')
    expect(visible(stdout.output)).not.toMatch(/\d+s/)

    stdout.output = ''
    vi.setSystemTime(new Date('2026-08-17T10:02:10.000Z'))
    await vi.advanceTimersByTimeAsync(SPINNER_PERIOD_MS)
    expect(visible(stdout.output)).not.toMatch(/\d+s/)
