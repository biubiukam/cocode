...
describe('resolveFooterHints', () => {
  ...
  it('projects draft, running, selection, focus, and confirmation states', () => {
    ...
    expect(resolveFooterHints({ agent: 'idle', draft: '', messageSelection: true }, keymap, 'en', 160).hints.map((hint) => hint.id)).toEqual(['message-move', 'message-actions', 'message-close'])
