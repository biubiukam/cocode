...
describe('message text selection', () => {
  ...
  it('copies the same text when the drag is reversed', () => {
    ...
    expect(selectedMessageText(nodes, backward)).toBe(selectedMessageText(nodes, forward))
  ...
  it('selects visible thinking text before the assistant reply', () => {
    ...
    expect(selectableNodeText({ ...assistant, thinkingDurationMs: 1250 })).toBe('thoughts\nanswer')
  ...
  it('maps a markdown list row to the item text, not the preceding paragraph', () => {
    const text = ['Here are some things I can assist with:', '', '- 🔍 Explore the codebase', '- 📝 Review code or changes'].join('\n')
    const assistant: ConversationNode = {
      kind: 'assistant',
      id: 'list',
      seq: 1,
    ...
    const cellColumn = stringWidth(`${glyphs.listBullet} `) + stringWidth(item.slice(0, item.indexOf('changes')))
