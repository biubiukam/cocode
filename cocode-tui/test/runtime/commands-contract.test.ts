...
describe('builtin command contract', () => {
  ...
  it('gates every runtime-backed command on its capability', () => {
    ...
    expect(registry.find('permission', all)?.name).toBe('permission')
    ...
    expect(registry.find('permission', { ...all, permissionMode: false })).toBeUndefined()
  ...
  it('dispatches every P0 local command to a concrete callback', () => {
    ...
    const ctx = {
      ...
      showEffortPicker: () => calls.add('showEffortPicker'),
