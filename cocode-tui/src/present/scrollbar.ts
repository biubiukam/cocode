...
export function scrollbarThumb(options: { trackRows: number; contentRows: number; scrollOffset: number }): ScrollbarThumb | undefined {
  ...
  const offset = Math.max(0, Math.min(maxOffset, Math.trunc(options.scrollOffset)))
  const size = Math.max(1, Math.min(track, Math.round((track * track) / content)))
