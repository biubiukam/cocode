...
export type ToolSummary = {
  ...
  elapsed?: string
  tone: 'accent' | 'success' | 'danger' | 'mute'
}
...
export function toolDisplayState(
  ...
  if (node.status === 'cancelled') {
    return {
      mark: glyphs.canceledMark,
      color: theme.mute,
      label: locale === 'zh' ? '已取消' : 'canceled',
    }
  }
  return {
...
export function projectToolSummary(
  ...
  const elapsedCandidate =
    node.status === 'running' ? formatElapsed(node.time, now) : undefined
  const chromeWidth =
    MESSAGE_CHROME + stringWidth(`${state.mark}  · ${state.label}`)
  const primaryReserve =
    primary === undefined
      ? 0
      : stringWidth(' · ') + Math.min(16, stringWidth(primary))
  const nameBudget = Math.max(
    1,
    Math.min(32, width - chromeWidth - primaryReserve),
  )
  const name = truncateCellWidth(rawName, nameBudget)
  const baseWidth =
    MESSAGE_CHROME + stringWidth(`${state.mark} ${name} · ${state.label}`)
  ...
  const elapsed =
    ...
    baseWidth + stringWidth(` · ${elapsedCandidate}`) + detailReserve <= width
      ...
      : undefined
  const usedWidth =
    baseWidth + (elapsed === undefined ? 0 : stringWidth(` · ${elapsed}`))
...
/** Same characters ToolCard paints on the compact summary row. */
export function joinToolSummary(summary: ToolSummary): string {
  let line = `${summary.mark} ${summary.name} · ${summary.statusLabel}`
  if (summary.elapsed !== undefined) line += ` · ${summary.elapsed}`
  if (summary.primaryDetail !== undefined)
    line += ` · ${summary.primaryDetail}`
  return line
}

export function formatToolSummaryLine(
  node: ToolNode,
  locale: 'en' | 'zh',
  columns: number,
  now = 0,
): string {
  return joinToolSummary(projectToolSummary(node, locale, columns, now))
}
...
function selectPrimaryDetail(node: ToolNode): string | undefined {
  ...
  const firstResultLine = node.result
    ?.replace(/\r\n?/g, '\n')
    .split('\n', 1)[0]
  if (node.status === 'error') {
