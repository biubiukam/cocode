...
import { localTextRange, type MessageTextRange } from '../message-text-selection.ts'
...
export function SelectableText(
  ...
  for (const entry of graphemeSegments(text)) {
    const nextSelected = entry.index >= selection.start && entry.index < selection.end
  ...
  return (
    <Box flexDirection="column" width={width} minWidth={0}>
      {wrapPlainText(text, width).map((line, index) => {
        ...
        return <SelectableText key={`${line.start}:${index}`} {...textProps} wrap="truncate-end" text={slice} selection={localTextRange(selection, line.start, slice.length)} />
      })}
    </Box>
  )
}
