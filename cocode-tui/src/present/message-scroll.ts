...
export function transcriptPaintColumns(
  nodes: readonly ConversationNode[],
  maxRows: number,
  verbose = false,
  expandedNodeIds: ReadonlySet<string> = EMPTY_EXPANDED_NODES,
  ...
  if (
    maxMessageScrollOffset(nodes, maxRows, verbose, expandedNodeIds, columns) <=
    0
  ) {
    return columns;
  }
...
export function scrollOffsetForMessage(
  ...
  const selectedIndex = nodes.findIndex(
    (node) => nodeKey(node.kind, node.id) === selectedNodeId,
  );
...
export function resolveMessageWindow(
  ...
  if (budget === 0 || nodes.length === 0)
    return { nodes: [], hiddenRowsBefore: 0 };
